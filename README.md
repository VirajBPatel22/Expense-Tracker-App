...
- 🛡️ **Multi-User Data Isolation**: Completely isolated financial records for each registered user.
...
### 3. Backend Setup
```bash
cd "Expence tracker app by viraj/backend"
npm install
```

Create a `.env` file in `Expence tracker app by viraj/backend`:
```env
PORT=5001
...
In a new terminal:
```bash
cd "Expence tracker app by viraj/frontend/app"
...
2. Set **Root Directory**: `Expence tracker app by viraj/backend`
...
2. Set **Root Directory**: `Expence tracker app by viraj/frontend/app`
